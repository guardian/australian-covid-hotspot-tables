...
export default async function colourize(headings, userKey, data, ColorScale) {
	...
	return await values.map((row, i) => row.map((value, index) => { return { value : value, color : colourizer(value, index), contrast : setContrast(colourizer(value, index)), meta : row.toString() }}))
}
