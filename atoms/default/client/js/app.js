...
// https://interactive.guim.co.uk/embed/aus/feeds/covid/nsw-hotspots.json
//https://data.nsw.gov.au/data/dataset/0a52e6c1-bc0b-48af-8b45-d791a6d8e289/resource/f3a28eed-8c2a-437b-8ac1-2dab3cf760f9/download/covid-case-locations-20210104b.json

fetch(`https://interactive.guim.co.uk/embed/aus/feeds/covid/nsw-hotspots.json?t=${new Date().getTime()}`)
    .then(res => res.json())
    .then(json => {
    	...
    	var results = json.map(item => {
    		return { 
    			"Suburb" : item.Suburb,
    			"Venue" : `${item.Venue} <br/>${item.Address}`,
    			"Date and time" : `${item.Time} on ${item.Date}`,
    			"Health advice" : (item.HealthAdviceHTML!=undefined) ? item.HealthAdviceHTML.replace(/(<([^>]+)>)/gi, "") : "" 
    		}
    	})
        ...
        data.sheets.data = groups.sort((a,b) => (a.Suburb > b.Suburb) ? 1 : ((b.Suburb > a.Suburb) ? -1 : 0)); 
